...
const eslintConfig = [
  ...
  // No ad-hoc utility classes, anywhere.
  //
  // The whole app (product pages, dev tooling, test pages) styles via CSS
  // Modules + the hand-written ui-kit. This block forbids utility-style class
  // tokens from appearing in any className. It is LAST so it wins over the
  // general TS block for matched files (flat config replaces, not merges,
  // no-restricted-syntax) — hence the arrow-function rule is repeated here.
  // It does NOT flag ui-kit globals (ui-*, c-*), custom z-index (z-modal), or
  // a11y helpers (focus-ring, sr-only). Tests are excluded so class-name
  // string literals in assertions (e.g. toHaveClass("...")) don't trip it.
  // For a genuine non-utility class that trips it, prefer a CSS Module; only
  // add an eslint-disable with a note if truly unavoidable.
  {
    ...
    rules: {
      "no-restricted-syntax": [
        ...
        {
          // className="…" string literals containing a utility-style class token.
          selector:
          ...
          message:
            "Utility-style class in product code. Style product code with a CSS Module or the ui-kit (ui-*/c-*). See app/.context/css-styles.md."
        }
      ]
    }
  }
