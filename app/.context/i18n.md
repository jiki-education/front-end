...
- **Message catalog**: `messages/en.json` is canonical; every other locale has a
  `messages/{locale}.json` alongside it. All catalogs are structurally identical to `en.json` (same
  key tree, same key count) — see [Untranslated stubs](#untranslated-stubs) for how not-yet-translated
...
- **Generation**: `scripts/generate-messages-cache.js` (wired into `dev` and `build`, with a
  ...
  `public/static/i18n/app/{locale}/messages-{hash}.json` plus the hash manifest
  `lib/generated/messages-hashes.ts` (locale -> hash). Uploaded to R2 immutably by
...
Top-level namespaces in `messages/en.json` (and every other catalog, which are structurally identical):
...
**A value of `�` (U+FFFD REPLACEMENT CHARACTER) is a deliberate stub, not corruption. Do not
"fix" it, and do not treat it as a bug to report.**

Every locale's catalog carries the full key tree from day one so it stays structurally identical to
`en.json`. Keys that haven't been translated yet hold a single `�` as their value. Translation
proceeds in batches, so at any given moment most locales have a large block of `�` values — that is
the expected steady state, not a broken file.

Consequences worth knowing:

- **Key-count and structural-diff checks pass on a barely-translated catalog.** `en.json` and
  `pt-PT.json` having the same 1257 keys says nothing about translation coverage. Coverage means
  counting values that are not `�`.
- **The bytes are real.** On disk these are literal `EF BF BD`, not a mojibake artifact of how a
  file was read. Seeing them in a hex dump or `git show` does not indicate an encoding bug.
- **This is per-key, not per-file.** A catalog is normally a mix: translated values, a few values
  legitimately identical to English (`"Premium"`, `"Jiki"`), and `�` for the rest.

When adding keys to `en.json`, add the same keys to every other catalog with `�` as the value.
...
- **Plain-`.ts` strings with no hook/await context**: none remain in the chrome surface. Anything
  ...
  registry, `components/coding-exercise/lib/i18n/editorMessages.ts`: `useExerciseLoader` seeds it on
  every render via `setEditorMessages` from its `useTranslations("codingExercise")` translator, and
  the plain modules call `editorMessage(key, values)`. Resolution happens per call rather than at
  construction time, so a locale change reaches copy baked into long-lived objects — the gutter
  markers are created once and reused for every redraw, so an eagerly-resolved title would freeze at
  mount locale. English defaults are baked in so behaviour is correct before seeding; tests that
  assert on this copy call `seedEditorMessages()` (`tests/test-utils/`) so they run against the real
  catalog instead of those fallbacks. A string that genuinely needs translating from a plain module
  should adopt one of these pass-a-key patterns rather than hardcoding English.
...
(`locales/pt-PT/`) and the URL segment. The cache-generation scripts derive the locale from
the filename verbatim, so a locale id that disagrees with its files in case resolves to
nothing — and `resolveLocaleRouting` would 308 the canonical URL away to the miscased one.
...
1. Add the locale code to `ALL_LOCALES` in `lib/locales.ts` (canonical BCP-47 casing, e.g.
   `pt-BR`)
2. Add its message catalog `messages/{locale}.json` (structurally identical to `en.json`, with
   `�` for every not-yet-translated value — see [Untranslated stubs](#untranslated-stubs))
...
4. Add translated content markdown files in the content package. The filename/directory casing
   must match the `ALL_LOCALES` id exactly, since the locale is derived from the filename.
