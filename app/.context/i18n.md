...
- **Server (SSR)**: `lib/i18n/request.ts` fetches `assetsUrl(appMessagesPath(locale, hash))`
  with a module-scope promise cache keyed `${locale}:${hash}` (reused across requests on a warm
  isolate), one retry on failure, then throws. A rejected fetch is evicted from the cache so the
  next request retries instead of caching the failure.
- **Client**: `ClientLocaleProvider` uses the same hashed URL and an equivalent promise cache
  when swapping locales client-side.
