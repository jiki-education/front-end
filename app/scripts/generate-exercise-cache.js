...
const BONUS_EXERCISES_PATH = path.join(__dirname, "../../curriculum/dist/exercises/bonus-exercises.json");
...
/**
 * Write the exercise -> level manifest.
 *
 * Which level an exercise belongs to is neither prose nor code: it does not vary
 * by locale and it does not vary by programming language, so it belongs in
 * neither index. It is emitted here, compiled into the build, for the same
 * reason `exerciseCodeIndexHashes` is: it changes only when this repo deploys.
 *
 * Folding it into the prose index instead would put it inside an artifact the
 * i18n repo republishes per locale from its own (code-free) source, which would
 * silently drop the field for every translated locale.
 *
 * The sitemap is the consumer: it pairs this with the ordered level registry and
 * LAST_PUBLISHED_LEVEL_SLUG to list only exercises that are actually live.
 */
function writeLevelManifest(exercises) {
  const body = Object.keys(exercises)
    .sort()
    .filter((slug) => typeof exercises[slug].metadata.levelId === "string")
    .map((slug) => `  ${JSON.stringify(slug)}: ${JSON.stringify(exercises[slug].metadata.levelId)}`)
    .join(",\n");

  const content = `// Auto-generated by scripts/generate-exercise-cache.js — DO NOT EDIT
export const exerciseLevels: Record<string, string | undefined> = {
${body}
};
`;

  writeFile(path.join(GENERATED_DIR, "exercise-levels.ts"), content);
}

/**
 * Write the bonus manifest: the slugs of exercises with at least one bonus task.
 *
 * Read from the curriculum build's dist/exercises/bonus-exercises.json, which is
 * produced by importing every exercise module (tasks live in scenarios.ts, so no
 * file this script parses carries the fact). Compiled in with no pointer, like
 * the level manifest: it varies by neither locale nor language. The dashboard
 * uses it to show a bonus star on exercise nodes.
 */
function writeBonusManifest() {
  // The curriculum's dev watcher only compiles TypeScript; the JSON is written
  // by its build. Locally a missing file just means no stars until the next
  // build, but a deploy without it would silently ship none, so CI fails.
  let slugs = [];
  if (fs.existsSync(BONUS_EXERCISES_PATH)) {
    slugs = JSON.parse(fs.readFileSync(BONUS_EXERCISES_PATH, "utf-8"));
  } else if (process.env.CI) {
    throw new Error(`Missing ${BONUS_EXERCISES_PATH}. Build the curriculum package first.`);
  } else {
    console.warn(`⚠️  ${BONUS_EXERCISES_PATH} not found; bonus manifest will be empty until the curriculum is built.`);
  }
  const body = slugs.map((slug) => `  ${JSON.stringify(slug)}`).join(",\n");

  const content = `// Auto-generated by scripts/generate-exercise-cache.js — DO NOT EDIT
export const exercisesWithBonus: ReadonlySet<string> = new Set([
${body}
]);
`;

  writeFile(path.join(GENERATED_DIR, "exercise-bonuses.ts"), content);
}
