  ...
  while (node) {
    if (isFunctionNode(node)) {
      ...
      if (fnName) {
        ...
        if (unFoldables?.includes(fnName.name)) {
          return null;
        }
      }
...
function getFunctionIdentifier(node: SyntaxNode, state: EditorState) {
...
export const foldGutterExtension = [
  ...
  foldGutter({
    ...
    foldingChanged: (update) => {
      const startUnfoldable = update.startState.field(unfoldableFunctionsField, false);
      const currentUnfoldable = update.state.field(unfoldableFunctionsField, false);
