...
export class PlaceholderWidget extends WidgetType {
  ...
  toDOM(_view: EditorView): HTMLElement {
    ...
    wrap.innerText = `${this.mode === "tutorial" ? tutorialEmoji : ""} ${this.label}`;
    return wrap;
  }
}
...
export class SVGWidget extends WidgetType {
  ...
  toDOM(_view: EditorView): HTMLElement {
    ...
    svgContainer.style.left = `${Math.min(this.hoverRight, this.declarationCoords.right)}px`;
    svgContainer.innerHTML = generateArrowSVG(this.arrowHeight, this.right, this.hoverRight);
    return svgContainer;
  }
}
...
function generateArrowSVG(length: number, right: number, hoverRight: number) {
  ...
  // const bl = `${hoverRight} ${length}`;
  // console.log("hover", hoverRight, "right", right);
  return `
  <svg height="${length + 10}" width="${right + Math.abs(hoverRight - right)}" stroke-width="2" stroke="black">
