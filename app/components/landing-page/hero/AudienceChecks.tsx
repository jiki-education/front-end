...
// on its own line.
const AUDIENCE_KEYS = ["audienceAbsoluteBeginners", "audienceRepeatBeginners", "audienceJuniors"] as const;

// The tagline's highlight draws first; each underline follows it by this much.
const UNDERLINE_STAGGER_SECONDS = 0.7;
        ...
        <li key={key} className={styles.check}>
          ...
          <span className={styles.text}>
            {t.rich(key, {
              ...
              u: (chunks) => (
                <DrawnMark type="underline" delay={UNDERLINE_STAGGER_SECONDS * (index + 1)}>
                  {chunks}
                </DrawnMark>
              )
            })}
