...
/** How long a shot is in flight, mirroring the `shot` animation; `stateAt` uses it to seek shots. */
export const SHOT_MS = 110;

/** One knob: every beat and CSS duration is divided by this to run the whole thing faster in step. */
export const SPEED = 1.15;

/** Time to let the pointer travel before the action it reached for fires, with a little settle on top. */
...
/** Every mutation the video performs, as a closed set. */
export type Action =
  | { type: "reset" }
  ...
  /** Whether the pointer shows the open hand; kept separate from `track` so it flips on arrival, not on setting off. */
  | { type: "grip"; gripping: boolean }
  | { type: "scrubbing"; scrubbing: boolean }
  | { type: "pressRun"; pressed: boolean }
  | { type: "pressSend"; pressed: boolean }
  /** A generic click flash for targets with no press state of their own — the tabs and the caret placement. */
...
/** Build the timeline. Called once at module scope; reads no DOM, so the result is the same every loop. */
function buildTimeline(): Timeline {
  ...
  // The previous loop's closing cards fade out here; drop their finale styling once transparent.
  push(CALLOUT_FADE_MS, { type: "endFinaleFade" });
  // The pointer rests by the code from the start, so the opening frame already has one on screen.
  ...
  // The pointer heads for Run while the green flash is still fading, rather than after a dead pause.
  t += 550;
  ...
  // Timed off the board-clearing shot rather than the `win` marker, so there's no dead air before the celebration.
  ...
  // The modal gets a beat on its own; the in-video cards are dismissed and, only once faded, all six arrive laid out in columns.
...
/**
 * Type the learner's reply into the chat box over a fixed number of ticks. The reply is sliced by
 * fraction rather than per character, so every locale finishes typing at the same moment.
 */
function pushTyping(push: Push, from: number) {
  const TICK = 55;
  const ticks = 11;

  push(from, { type: "cursor", target: { kind: "chatBox" } });
  const startsAt = from + REACH; // let the cursor land before a key is pressed
