...
export default async function AppConceptPage({ params }: Props) {
  ...
  const concept = await getConceptServer(slug);
  const ancestors = concept ? await getAncestorsServer(slug) : [];
  const isCategory = concept?.category ?? false;
  const initialSubconcepts = isCategory ? await getChildrenServer(slug) : undefined;
  ...
  if (concept && !isCategory) {
    const [content, relatedConcepts, relatedExercises, videoData] = await Promise.all([
      getConceptContentServer(slug),
      getRelatedConceptsServer(slug),
      getExercisesForConceptServer(slug),
