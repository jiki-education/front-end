...
describe("authStore.recheckAuth", () => {
  const mockUser: User = {
    handle: "testuser",
    email: "test@example.com",
    name: "Test User",
  ...
  beforeEach(() => {
    useAuthStore.setState({ user: null, isAuthenticated: false, isLoading: false, error: null, hasCheckedAuth: true });
    jest.clearAllMocks();
  });

  it("asks the API again even though the store has already been checked", async () => {
    mockGetCurrentUser.mockResolvedValue(mockUser);

    await useAuthStore.getState().recheckAuth();
  ...
  it("does nothing while a check is already in flight", async () => {
    useAuthStore.setState({ isLoading: true });
