...
import EpisodeVideo from "@/components/projects/EpisodeVideo";
...
jest.mock("@/components/projects/lib/useEpisodeProgress", () => ({
  useEpisodeProgress: () => ({
...
const baseProps = {
  ...
  projectPath: "/projects/building-basics",
  videoKey: "abc123"
} as const;

describe("EpisodeVideo", () => {
  beforeEach(() => {
    jest.clearAllMocks();
  });
  ...
  it("renders the Mux player for mux episodes", () => {
    ...
    render(<EpisodeVideo {...baseProps} videoProvider="mux" premium={false} />);
  ...
  it("renders the YouTube player for youtube episodes", () => {
    ...
    render(<EpisodeVideo {...baseProps} videoProvider="youtube" premium={false} />);
  ...
  it("redirects non-premium users from premium episodes and opens the upgrade modal", () => {
    ...
    render(<EpisodeVideo {...baseProps} videoProvider="mux" premium={true} />);

    expect(showPremiumUpgradeModal).toHaveBeenCalledWith("locked_episode_video", expect.any(Object));
    expect(mockReplace).toHaveBeenCalledWith("/projects/building-basics");
