...
describe("LocaleSwitcher", () => {
  ...
  // The options must be the locales this build serves, so the picker can never
  // offer one whose catalog ClientLocaleProvider cannot load.
  it("offers exactly the locales this build serves, plus the auto option", () => {
    setUser(createMockUser());
    render(<LocaleSwitcher />);
    const values = screen.getAllByRole("option").map((option) => (option as HTMLOptionElement).value);
    // The leading "" is the disabled auto option (no explicit choice made).
