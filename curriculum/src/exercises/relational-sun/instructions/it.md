...
In questo esercizio, il tuo compito è posizionare un sole nell'angolo in alto a destra del cielo usando aritmetica e variabili. Il vincolo principale è che il bordo del sole deve rimanere sempre a una distanza fissa dal bordo dell'area di disegno, indipendentemente dalla dimensione del sole.
...
Tuttavia, per superare l'esercizio non puoi semplicemente «scrivere un valore fisso» (cioè impostarlo su un numero singolo): `sunX` e `sunY` devono essere calcoli che coinvolgono altre variabili.

Una volta superato l'esercizio, premi "Tidy my code" e prova a cambiare i valori di `gap` e `radius` all'inizio del file, quindi esegui di nuovo il codice per vedere il sole ingrandirsi o spostarsi rimanendo però nell'angolo.
