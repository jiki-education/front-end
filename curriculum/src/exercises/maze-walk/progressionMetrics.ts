...
function pathProgress(ex: MazeWalkExercise | undefined): number {
  ...
  metrics: [
    {
      ...
      maxScore: 1,
      points: 4,
      score: (runs) => pathProgress(runs.bySlug(SCENARIO_SLUG)?.exercise as MazeWalkExercise | undefined)
    },
    {
      // The exercise's headline concept: walk's body repeats move() with a
      // loop driven by the numSteps input. A loop turns one or two static
      // move() calls into many runtime moves; a hand-unrolled or hardcoded
      // body needs a static call per step.
      name: "used_loop_in_walk",
      maxScore: 1,
      points: 8,
      score: (runs) => {
        ...
        const runtimeMoves = run.result.meta.functionCallLog.filter((entry) => entry.name === "move").length;
        return staticMoves >= 1 && staticMoves <= 2 && runtimeMoves >= 5 ? 1 : 0;
      }
    }
  ]
};
