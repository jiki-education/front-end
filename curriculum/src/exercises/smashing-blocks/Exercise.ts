...
export default class SmashingBlocksExercise extends VisualExercise {
  protected get slug() {
  ...
  private blockInstances: Array<jikiscript.Instance | javascript.JSInstance> = [];
  ...
  // Configurable defaults for bonus scenario
  private defaultBallRadius = 3;
  private defaultBlockHeight = 7;

  public setDefaultBallRadius(radius: number) {
    this.defaultBallRadius = radius;
  }

  public setDefaultBlockHeight(height: number) {
  ...
  private createJikiscriptBallClass(): jikiscript.Class {
    ...
    const radius = this.defaultBallRadius;
    BallClass.addConstructor((_ctx: ExecutionContext, obj: jikiscript.Instance) => {
      ...
      obj.setField("radius", new jikiscript.Number(radius));
  ...
  private createJSBallClass(): javascript.JSClass {
    ...
    BallClass.addConstructor((_ctx: ExecutionContext, obj: javascript.JSInstance) => {
      ...
      obj.setField("radius", new javascript.JSNumber(radius));
  ...
  private createJikiscriptBlockClass(): jikiscript.Class {
    const BlockClass = new jikiscript.Class("Block");

    BlockClass.addProperty("left");
    BlockClass.addProperty("top");
    BlockClass.addProperty("width");
    BlockClass.addProperty("height");
    BlockClass.addProperty("smashed");

    const blockHeight = this.defaultBlockHeight;
    const exercise = this;
    BlockClass.addConstructor(
      (_ctx: ExecutionContext, obj: jikiscript.Instance, leftArg: Shared.JikiObject, topArg: Shared.JikiObject) => {
        const left = (leftArg as jikiscript.Number).value;
        const top = (topArg as jikiscript.Number).value;
        obj.setField("left", new jikiscript.Number(left));
        obj.setField("top", new jikiscript.Number(top));
        obj.setField("width", new jikiscript.Number(16));
        obj.setField("height", new jikiscript.Number(blockHeight));
        obj.setField("smashed", new jikiscript.Boolean(false));
        exercise.blockInstances.push(obj);
      }
    );

    BlockClass.addGetter("left", "public");
    BlockClass.addGetter("top", "public");
    BlockClass.addGetter("width", "public");
    BlockClass.addGetter("height", "public");
    BlockClass.addGetter("smashed", "public");
    BlockClass.addSetter("smashed", "public");

    return BlockClass;
  }

  private createJSBlockClass(): javascript.JSClass {
    const BlockClass = new javascript.JSClass("Block");
    ...
    const exercise = this;
    BlockClass.addConstructor(
      (_ctx: ExecutionContext, obj: javascript.JSInstance, leftArg: Shared.JikiObject, topArg: Shared.JikiObject) => {
        const left = (leftArg as javascript.JSNumber).value;
        const top = (topArg as javascript.JSNumber).value;
        obj.setField("left", new javascript.JSNumber(left));
        obj.setField("top", new javascript.JSNumber(top));
        obj.setField("width", new javascript.JSNumber(16));
        obj.setField("height", new javascript.JSNumber(blockHeight));
        obj.setField("smashed", new javascript.JSBoolean(false));
        exercise.blockInstances.push(obj);
      }
    );
  ...
  getExternalClasses(language: Language): any[] {
    if (language === "jikiscript") {
      ...
      return [this.createJikiscriptBallClass(), this.createJikiscriptBlockClass()];
    }
    if (language === "javascript") {
      this.velocityFieldX = "xVelocity";
      this.velocityFieldY = "yVelocity";
      return [this.createJSBallClass(), this.createJSBlockClass()];
    }
    return [];
  }

  getState() {
    const numSmashedBlocks = this.blockInstances.filter((inst) => {
      return inst.getUnwrappedField("smashed") === true;
    }).length;

    return {
      numBlocks: this.blockInstances.length,
      numSmashedBlocks,
      numBallPositions: this.moveBallCallCount
    };
  }
