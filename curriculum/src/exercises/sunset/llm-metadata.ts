  ...
  tasks: {
    "draw-scene": {
      description: `
        ...
        - Sun color: animate using RGB - decrease green from 237 to make it more orange
        - Sky color: animate using HSL - change hue to shift the color

        Key teaching points:
        1. Variables must be set OUTSIDE the loop (set/let)
        2. Variables must be changed INSIDE the loop BEFORE drawing (change/=)
        ...
        Common mistakes:
        - Setting variables inside the loop (resets them each iteration)
        - Changing variables after drawing (one frame behind)
        ...
        Solution approach:
        1. Set sun_radius, sun_cy, sun_green (and optionally sky_h) before the loop
        2. In each iteration: change variables first, then draw sky, sun, sea, sand
      `
    }
  }
};
