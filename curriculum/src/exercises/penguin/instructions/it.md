...
In questo esercizio disegniamo per la prima volta delle ellissi e riflettiamo un po' di più sulla simmetria che abbiamo menzionato in Foxy Face.
...
La funzione `ellipse` prende 5 input: la posizione del centro (`centerX` (coordinata X del centro), `centerY` (coordinata Y del centro)), il raggio orizzontale (`radiusX` (raggio orizzontale)), il raggio verticale (`radiusY` (raggio verticale)), e il colore:
...
**Inizia premendo _"Esegui codice"_** per vedere come appare l'immagine di partenza. Divertiti!
