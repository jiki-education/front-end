...
Nel primo esercizio sul labirinto hai spostato il tuo personaggio manualmente attraverso il labirinto. Ora sei pronto a risolvere qualsiasi labirinto in modo programmatico usando il codice!

Per rendere tutto questo possibile, hai tre nuove funzioni:

- `canTurnLeft()` (può girare a sinistra): restituisce `true` se lo spazio a sinistra del personaggio non è un muro.
...
Quando le abbiniamo a `move()` (muovi), `turnLeft()` (gira a sinistra) e `turnRight()` (gira a destra) che avevi già, è possibile scrivere un algoritmo per risolvere **qualsiasi** labirinto. «Algoritmo» è una parola ricercata che significa semplicemente «un modo di fare qualcosa». È come una formula o una ricetta.
...
Una volta capito, o se ti arrendi, scorri verso il basso per vedere la soluzione e quello che devi scrivere nel codice.
