...
const SCENARIO_SLUG = "maze-repeat-1";
const MOVE_CALL_NAMES = ["move"];
...
function pathProgress(ex: MazeSolveRepeatExercise | undefined): number {
...
export const progressionMetrics: ProgressionMetrics = {
  ...
  metrics: [
    {
      // The stub already solves the maze; this tracks whether refactoring
      // edits have broken the route (and how far the character still gets).
      name: "path_progress",
      maxScore: 1,
      points: 4,
      score: (runs) => pathProgress(runs.bySlug(SCENARIO_SLUG)?.exercise as MazeSolveRepeatExercise | undefined)
    },
    {
      // The exercise's headline concept: collapsing runs of move() calls
      // into repeat loops. Loops make the runtime move count outstrip the
      // static call count; the smallest group worth collapsing (5 moves)
      // already produces a surplus of 4.
      name: "used_repeat",
      maxScore: 1,
      points: 8,
      score: (runs) => {
        ...
        const staticMoves = run.result.assertors.numFunctionCallsInCode("move");
        const runtimeMoves = run.result.meta.functionCallLog.filter((entry) =>
          MOVE_CALL_NAMES.includes(entry.name)
        ).length;
        return staticMoves >= 1 && runtimeMoves >= staticMoves + 4 ? 1 : 0;
      }
    }
  ]
};
