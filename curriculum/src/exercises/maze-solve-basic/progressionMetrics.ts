...
function pathProgress(ex: MazeSolveBasicExercise | undefined): number {
...
export const progressionMetrics: ProgressionMetrics = {
  ...
  metrics: [
    {
      // Partial progress through the maze on a failing run - how much closer
      // to the target the character finished than where it started.
      name: "path_progress",
      maxScore: 1,
      points: 5,
      score: (runs) => pathProgress(runs.bySlug(SCENARIO_SLUG)?.exercise as MazeSolveBasicExercise | undefined)
    }
  ]
};
