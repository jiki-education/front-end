---
title: "هزارتو را حل کنید"
description: "با دستورهای ساده، Jiki را در هزارتو راهنمایی کنید."
en_md5: bbd838559fba4dcf527a55ba135703cd
---

به اولین تمرین‌تان خوش آمدید!

هدف این تمرین این است که با نحوه‌ی کار محیط یادگیری آشنا شوید. کار شما این است که با دادن دستور به آدمک، هزارتویی را که در سمت چپ می‌بینید حل کنید. همه‌ی دستورهایی را که آدمک باید دنبال کند می‌نویسید، بعد روی **اجرای کد** کلیک می‌کنید تا آن‌ها را انجام دهد.

سه دستوری که می‌توانید استفاده کنید این‌ها هستند:

- `move()` (حرکت کن) که آدمک را یک قدم به جلو می‌برد
- `turnLeft()` (به چپ بچرخ) آدمک را به چپ می‌چرخاند (نسبت به جهتی که در حال حاضر رو به آن است)
- `turnRight()` (به راست بچرخ) آدمک را به راست می‌چرخاند (نسبت به جهتی که در حال حاضر رو به آن است)

می‌بینید که ما سه دستور اول را از قبل در سمت چپ نوشته‌ایم. اول روی دکمه‌ی **«اجرای کد»** کلیک کنید تا ببینید چه کار می‌کنند. بعد **دستورهای بیشتری اضافه کنید** تا آدمک‌تان به انتهای هزارتو برسد. هر دستور را در یک خط جداگانه بنویسید و بعد **اجرای کد** را بزنید تا همه‌ی آن‌ها اجرا شوند.

خوب است عادت کنید که کدتان را مرتب اجرا کنید!

وقتی هزارتو را حل کردید، می‌توانید به داشبورد برگردید و درس بعدی را شروع کنید.
