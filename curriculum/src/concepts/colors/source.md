...
Now let's look at HSL, which gives us a completely different way to think about colors. HSL stands for <define>hue</define>, <define>saturation</define>, and <define>lightness</define>. And similarly to RGB, you have a function called `hsl` that takes three numbers, one for hue, one for saturation, and one for lightness, and it will return out a color. So let's break down what those three words mean.
