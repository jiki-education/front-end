...
- `alphanumeric` - Alphanumeric: classify strings as alpha, numeric, alphanumeric, or unknown using multiple helper functions (Advanced Loops level)
...
- `caesar-cipher` - Caesar Cipher: encode messages by shifting letters in the alphabet (Advanced Loops level)
- `acronym` - Acronym: convert a phrase to its acronym (Multiple Functions level)
