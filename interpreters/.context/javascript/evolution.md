...
- **HTML lives inside the catalog values**; only the dynamic pieces are `{{placeholders}}` (the results of `codeTag(...)`/`formatJSObject(...)` are passed as params — `escapeValue: false` keeps their HTML intact). List-joining/glue (`", "`, `" and "`) and English ordinals (`addOrdinalSuffix`) stay in TS and are passed as assembled params. Pure count-pluralization uses i18next `_one`/`_other` keys (e.g. `expressionStatement.args_*`, `arrayExpression.created_*`, `blockStatement.*_other`).
