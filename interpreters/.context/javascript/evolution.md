...
That only works if i18next knows the real locale: `Intl.PluralRules` drives the choice, and the previous synthetic `lng: "x"` has no CLDR data at all, so it degraded to a crude one/other split (English itself rendered "21th"). The locale therefore travels **with** the dict, under the reserved `$locale` key (`tagLocale()` in `src/shared/i18n.ts`), stamped at the app's fetch boundary rather than threaded as a second value down every call in the app's run rail. An untagged dict resolves as `en`, which is right for the bundled default and loud for anything else. Locale codes are canonicalised before use (`pt-pt` → `pt-PT`), since i18next normalises on resolve but not when reading `resources`. On-disk catalogs never carry `$locale`.
...
- **`DescriptionContext` gains a translator.** A JS-local `describers/types.ts` extends the shared `DescriptionContext` with `t: Translator`. `frameDescribers.describeFrame` always populates it (from the executor's `this.translate`, or `buildTranslator()`'s default when no context is passed), so every describer resolves prose via `context.t("description.<describer>.<variant>", { …params })`. The shared `DescriptionContext` (used by JikiScript/Python) is untouched — this is JS-only.
- **Catalog values are bare sentences; the renderer owns the block markup.** `frameDescribers` wraps the `Description.result` in `<p>` and each `Description.steps` entry in `<li>`, so translators never reproduce `<li>`/`<p>` boilerplate. (This also fixed a latent bug: the array/member/template expression describers used to emit bare step strings with no `<li>`, rendering as unwrapped text inside the `<ul>` — now every step is wrapped uniformly.) Inline markup (`<code>…</code>` from `codeTag(...)`/`formatJSObject(...)`) is still passed as `{{placeholder}}` params — `escapeValue: false` keeps it intact. List-joining/glue (`", "`, `" and "`) and English ordinals (`addOrdinalSuffix`) stay in TS as assembled params. Pure count-pluralization uses i18next `_one`/`_other` keys (e.g. `expressionStatement.args_*`, `arrayExpression.created_*`, `blockStatement.*_other`). The shared `"a function"` fallback lives in one key, `description.common.defaultFunctionName`.
- **Bundled default composes `system` + `en`.** `translator.ts`'s no-injection default is now `{ ...systemMessages, description: enMessages.description }`: errors keep the structured `system` canary, but descriptions (compositional prose with no meaningful canary form) are authored **once** in `en` and layered on top. So the no-injection path (tests/dev) still renders English descriptions while forgotten injections still surface loud canary _errors_. `system` deliberately holds **no** `description.*` keys; production always injects the active locale's full dict.
- **`hu` held at parity as English stubs** (copied from `en`, not translated). The translations guard (`tests/javascript/translations.test.ts`) was tightened from a one-directional subset check to **exact** en↔human-locale key parity (missing _and_ orphan keys fail), so a new `en` key can't silently ship absent from a live locale. `system` is exempt (errors-only base).
- `describeArrayExpression`/`describeMemberExpression`/`describeDictionaryExpression` now receive `context` (threaded through `describeSteps`); the three break/continue/return describers' previously-unused context params are now used. Dead code (`describeUpdateExpression`, `describeOperator`) was left untouched — its strings never reach a student.
...
- **Shared factory**: `src/shared/i18n.ts` exports `createTranslator(dict)` (fresh `createInstance`, `lng: "x"`, `fallbackLng: false`, `escapeValue: false`) — reusable by the other interpreters later. (`lng: "x"` was replaced by the dict's own `$locale` on 2026-08-02; see the top of this file.)
