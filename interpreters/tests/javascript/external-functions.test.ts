...
describe("JavaScript External Functions", () => {
  ...
  describe("snake_case to camelCase conversion", () => {
    it("should convert snake_case external function names to camelCase", () => {
      const externalFunction: ExternalFunction = {
        name: "turn_left",
        func: (context: ExecutionContext) => "turned",
        description: "turns the character left",
        arity: 0,
      };

      const result = interpret(`turnLeft();`, {
        externalFunctions: [externalFunction],
      });
    ...
    it("should convert multi-underscore names to camelCase", () => {
      const externalFunction: ExternalFunction = {
        name: "get_player_score",
        func: (context: ExecutionContext) => 100,
        description: "gets the player score",
        arity: 0,
      };

      const result = interpret(`getPlayerScore();`, {
        externalFunctions: [externalFunction],
      });
    ...
    it("should protect the camelCase version of the name", () => {
      const externalFunction: ExternalFunction = {
        name: "turn_left",
        func: (context: ExecutionContext) => "turned",
        description: "turns the character left",
      ...
      const result = interpret(`function turnLeft() { return "override"; }`, {
        externalFunctions: [externalFunction],
      });
      ...
      expect((result.frames[0] as any).error.message).toBe("FunctionAlreadyDefined: name: turnLeft");
    });
