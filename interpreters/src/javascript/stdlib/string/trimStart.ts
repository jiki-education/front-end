...
export const trimStart: Method = {
  arity: 0,
  call: (_ctx: ExecutionContext, obj: JikiObject, args: JikiObject[]) => {
    const str = obj as JSString;

    // Validate no arguments
    guardNoArgs(args, "trimStart");

    return new JSStringClass(str.value.trimStart());
  },
  description: "returns a string with whitespace removed from the beginning",
};
